...
from src.helper import LogDataAccessObject
...
class LogManager:
    ...
    def read(self, string_operation):
        """
        read-only commands to show or get already existing data - are not saved in logs because
        it actually makes no sense to store since that would save us memory and read-only logs
        can not cause conflict
        """
    ...
    def update_server_cluster(self, command):
        """
        Allows us to register and deregister server(s) from the cluster
        and saves the process of halting and booting up the server again
        """
        operands = command.split(" ")
        index, term, command, key, values = 0, 1, 2, 3, 4
        ...
        if operands[command] == "register":
            ...
            self.voted_for_me.pop(operands[key], "I am not even eligible to vote") #Skips KeyError in case a non-voting server is being removed

    def destination_addresses(self, server_name):
        other_servers = {}
        for k,v in self.server_cluster.items():
            if k != server_name:
                other_servers[k] = v
        return [val for val in other_servers.values()]
    ...
    def other_server_names(self, server_name):
        ...
        for k,v in self.server_cluster.items():
            if k != server_name:
                other_servers[k] = v
        return [val for val in other_servers.keys()]

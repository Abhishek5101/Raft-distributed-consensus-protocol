...
from src.helper import AppendEntriesCall
from src.helper import RequestVoteCall
...
class Server:
    def __init__(self, name, port=10000, voting=True):
        ...
        self.latest_leader = "Not elected"

        self.leader, self.voting, self.heartbeat_timer = False, voting, None

        self.followers_with_update_status, self.current_operation, self.current_operation_committed = {}, '', False

        self.timeout = float(random.randint(10, 18))
        self.election_countdown = threading.Timer(self.timeout, self.start_election)
        print("Timeout in : " + str(self.timeout))
    ...
    def start_election(self):
        if not self.leader:
            ...
            self.timeout = float(random.randint(10, 18))
            self.election_countdown = threading.Timer(self.timeout, self.start_election)
            print("New election timeout : " + str(self.timeout))
            self.election_countdown.start()
    ...
    def mark_updated(self, server_name):
        ...
        ayes = 0 
        for follower in self.followers_with_update_status.values():
            if follower is True:
                ayes += 1
    ...
    def mark_voted(self, server_name):
        ...
        ayes = 0 
        for follower in self.log_manager.voted_for_me.values():
            if follower is True:
                ayes += 1
